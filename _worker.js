...
export default {
  async fetch(请求735, 本地值734, 本地值733) {
    try {
      ...
      启用链路降级 = !!(值控制711 && 值控制711.toLowerCase() === 'no');
...
async function 处理值值384(地址类型383, 主机, 端口数字, 原始数据, 网页套接字382, 值头部381, 远程连接值, 请求回退 = '', 请求地区 = '', 请求值380 = null, 请求链路配置 = null, 请求值379 = null) {
  ...
  async function 连接值发送(地址377, 端口376, 值链路 = false) {
    const 远程值375 = 值链路 ? await 处理值链路连接(地址类型383, 地址377, 端口376, 实际链路配置) : await 连接值套接字(地址377, 端口376, 请求值379, 传输连接竞速数);
  ...
  try {
    const {
      remoteSock: 值套接字358,
      writer: 值写入器
    } = await 连接值发送(主机, 端口数字, 启用链路降级 ? false : 实际链路已启用);
    处理值远程(值套接字358, 值写入器, () => {
      处理值值当前(值套接字358, 值写入器);
      处理重试连接();
    });
...
async function 处理值链路连接(地址类型, 地址262, 端口261, 链路配置 = 已解析链路5配置) {
  const {
    username: 本地值260,
    password: 密码259,
    hostname: 主机名258,
...
function 解析链路配置(地址249) {
  ...
  链路端口 = Number(本地值242.pop());
  if (isNaN(链路端口)) throw new Error(错误_无效链路地址);
  主机名244 = 本地值242.join(":");
                    ...
                    <form id="otherConfigForm" style="margin-bottom: 20px;">
                        <div style="margin-bottom: 15px;">
                            ...
                            <div style="padding: 15px; background: rgba(15, 3, 40, 0.6); border: 1px solid #00f0ff; border-radius: 5px;">
                                <div style="margin-bottom: 10px;">
                                    <label style="display: inline-flex; align-items: center; cursor: pointer; color: #00f0ff;">
                                        <input type="checkbox" id="ev" checked style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                            <span style="font-size: 1.1rem;">${翻译值.enabletrans}</span>
                                    </label>
                                </div>
                                <div style="margin-bottom: 10px;">
                                    <label style="display: inline-flex; align-items: center; cursor: pointer; color: #00f0ff;">
                                        <input type="checkbox" id="et" style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                            <span style="font-size: 1.1rem;">${翻译值.enableconn}</span>
                                    </label>
                                </div>
                                <div style="margin-bottom: 10px;">
                                    <label style="display: inline-flex; align-items: center; cursor: pointer; color: #00f0ff;">
                                ...
                                <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid rgba(0, 240, 255, 0.3);">
                                        <label style="display: block; margin-bottom: 8px; color: #00f0ff; font-size: 0.95rem;">${翻译值.connPassword}</label>
                                        <input type="text" id="tp" placeholder="${翻译值.connPasswordPlaceholder}" style="width: 100%; padding: 10px; background: rgba(0, 0, 0, 0.8); border: 1px solid #00f0ff; color: #00f0ff; font-family: 'Courier New', monospace; font-size: 13px;">
                                        <small style="color: #7aa9c4; font-size: 0.8rem; display: block; margin-top: 5px;">${翻译值.connPasswordHint}</small>
                                </div>
...
function 更新配置值() {
  ...
  启用链路降级 = !!(有效配置.qj && 有效配置.qj.toLowerCase() === 'no');
  自定义路径 = 有效配置.d || '';
  优选地址源 = 有效配置.yxURL || '';
