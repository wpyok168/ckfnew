...
const 错误_无效链路地址 = atob('SW52YWxpZCBTT0NLUyBhZGRyZXNzIGZvcm1hdA==');
const 错误_链路无可用方法 = atob('bm8gYWNjZXB0YWJsZSBtZXRob2Rz');
const 错误_链路需要认证 = atob('c29ja3Mgc2VydmVyIG5lZWRzIGF1dGg=');
const 错误_链路认证失败 = atob('ZmFpbCB0byBhdXRoIHNvY2tzIHNlcnZlcg==');
const 错误_链路连接失败 = atob('ZmFpbCB0byBvcGVuIHNvY2tzIGNvbm5lY3Rpb24=');
const 错误_链路隧道失败 = atob('ZmFpbCB0byBvcGVuIHByb3h5IHR1bm5lbA==');
const 错误_链路响应异常 = atob('aW52YWxpZCBwcm94eSByZXNwb25zZQ==');
...
const 文本_链路认证头 = atob('UHJveHktQXV0aG9yaXphdGlvbjogQmFzaWMg');
const 文本_链路保持 = atob('UHJveHktQ29ubmVjdGlvbjogS2VlcC1BbGl2ZQ==');
const 文本_用户链路头 = atob('VXNlci1BZ2VudDogTW96aWxsYS81LjA=');
...
export default {
  async fetch(请求735, 本地值734, 本地值733) {
    try {
      ...
      链路5配置 = 获取配置文本值('s', 配置默认值.s, 本地值734.s || 本地值734.S);
      if (链路5配置) {
        try {
          已解析链路5配置 = 解析链路配置(链路5配置);
          是否链路已启用 = true;
        } catch (错误722) {
          是否链路已启用 = false;
        }
...
async function 处理网页套接字请求(请求417) {
  ...
  const 请求链路字符串 = 请求网址.searchParams.get('s') || '';
  let 请求链路配置413 = null;
  if (请求链路字符串) {
    try {
      请求链路配置413 = 解析链路配置(请求链路字符串);
    } catch (忽略值412) {}
  }
      ...
      if (!协议类型) {
        if (启用明文 && 数据396.byteLength >= 24) {
          ...
          if (!轻量协议结果.hasError) {
            ...
            await 处理值值384(地址类型395, 主机名393, 端口394, 原始数据389, 值值410, 值头部390, 远程连接值409, 请求回退416, 实际地区411, 请求值414, 请求链路配置413, 请求值407);
            return;
          }
        }
        if (启用木马 && 数据396.byteLength >= 56) {
          ...
          if (!值结果.hasError) {
            ...
            await 处理值值384(地址类型388, 主机名386, 端口387, 原始客户端数据, 值值410, null, 远程连接值409, 请求回退416, 实际地区411, 请求值414, 请求链路配置413, 请求值407);
            return;
          }
        }
...
async function 处理值值384(地址类型383, 主机, 端口数字, 原始数据, 网页套接字382, 值头部381, 远程连接值, 请求回退 = '', 请求地区 = '', 请求值380 = null, 请求链路配置 = null, 请求值379 = null) {
  // 优先使用客户端path参数，其次回退到全局配置
  const 实际回退 = 请求回退 || 回退地址;
  const 实际地区 = 请求地区 || 当前工作器地区;
  const 实际地区匹配 = 请求值380 !== null ? 请求值380 : 启用地区匹配;
  const 实际链路配置 = 请求链路配置 || 已解析链路5配置;
  const 实际链路已启用 = 请求链路配置 ? true : 是否链路已启用;
  const 值数据378 = 处理值值8数组(原始数据);
  async function 连接值发送(地址377, 端口376, 值链路 = false) {
    // 走链路时首包交给握手函数在释放写入器前发出，避免换写入器导致连接被重置
    const 远程值375 = 值链路 ? await 处理值链路连接(地址类型383, 地址377, 端口376, 实际链路配置, 请求值379, 值数据378) : await 连接值套接字(地址377, 端口376, 请求值379, 传输连接竞速数);
    const 写入器374 = 远程值375.writable.getWriter();
    if (!值链路 && 值数据378.byteLength) await 写入器374.write(值数据378);
    return {
      remoteSock: 远程值375,
      writer: 写入器374
    };
  ...
  try {
    // 首跳是否走链路：只走链路 → 必走；优先直连 → 不走；其余按链路是否配置
    const 首跳走链路 = 仅走链路 && 实际链路已启用 ? true : 启用链路降级 ? false : 实际链路已启用;
    const {
      remoteSock: 值套接字358,
      writer: 值写入器
    } = await 连接值发送(主机, 端口数字, 首跳走链路);
    处理值远程(值套接字358, 值写入器, () => {
      处理值值当前(值套接字358, 值写入器);
      处理重试连接();
    });
...
async function 处理值链路连接(地址类型, 地址262, 端口261, 链路配置 = 已解析链路5配置, 请求值258 = null, 首包数据 = null) {
  // 按链路种类分派：隧道走建隧请求，其余保持套接字5 握手
  if (链路配置 && (链路配置.kind === 链路种类_隧道 || 链路配置.kind === 链路种类_安全隧道)) {
    return 处理值隧道连接(地址262, 端口261, 链路配置, 请求值258, 首包数据);
  }
  const {
    username: 本地值260,
    password: 密码259,
  ...
  const 套接字256 = 处理打开值套接字(主机名258, 链路端口257, 请求值258);
  ...
  async function 读满(需要长度) {
    while (残留字节.length < 需要长度) {
      ...
      if (已结束 || !分片) throw new Error(错误_链路连接失败);
      残留字节 = 拼接值8数组(残留字节, 分片);
    }
    return 残留字节;
  }
  ...
  if (本地值253[0] !== 5 || 本地值253[1] === 255) throw new Error(错误_链路无可用方法);
  const 选中方法 = 本地值253[1];
  取走(2);
  if (选中方法 === 2) {
    if (!本地值260 || !密码259) throw new Error(错误_链路需要认证);
    ...
    本地值253 = await 读满(2);
    if (本地值253[0] !== 1 || 本地值253[1] !== 0) throw new Error(错误_链路认证失败);
    取走(2);
  }
  // 统一用域名型寻址：调用方的地址类型编号在不同协议下含义不一致（值协议 2=域名，
  // 木马协议 3=域名），按编号分支会把域名当成六版地址编错。交给链路自己解析更稳。
  ...
  本地值253 = await 读满(4);
  if (本地值253[1] !== 0) throw new Error(错误_链路连接失败);
...
async function 处理值隧道连接(地址238值, 端口237值, 链路配置, 请求值236值 = null, 首包数据235值 = null) {
  const {
    username: 隧道用户,
    password: 隧道密码,
    hostname: 隧道主机,
  ...
  let 请求头 = `${文本_连接方法} ${目标地址}${文本_协议版本}${文本_换行}` + `${文本_主机头}${目标地址}${文本_换行}` + `${文本_用户链路头}${文本_换行}` + `${文本_链路保持}${文本_换行}`;
  if (隧道用户) {
    请求头 += `${文本_链路认证头}${btoa(`${隧道用户}:${隧道密码 || ''}`)}${文本_换行}`;
  }
  请求头 += 文本_换行;
  ...
  try {
    ...
    while (头部结束 < 0) {
      ...
      if (已结束 || !分片) throw new Error(错误_链路隧道失败);
      缓冲 = 拼接值8数组(缓冲, 分片);
      ...
      if (头部结束 < 0 && 缓冲.length > 8192) throw new Error(错误_链路响应异常);
    }
    const 状态行 = 共享解码器.decode(缓冲.subarray(0, Math.min(头部结束, 128)));
    if (!状态行.startsWith(文本_响应前缀)) throw new Error(错误_链路响应异常);
    const 状态码 = Number(状态行.split(' ')[1]);
    if (!(状态码 >= 200 && 状态码 < 300)) throw new Error(错误_链路隧道失败);
    // 链路在头部之后可能已经捎带了目标数据，需要交还给下游
...
function 解析链路配置(地址249) {
  let 剩余地址 = String(地址249 || '').trim();
  // 按前缀识别链路种类，无前缀保持原有行为（套接字5）
  let 链路种类 = 链路种类_套接字5;
  ...
  if (!剩余地址) throw new Error(错误_无效链路地址);
  let [本地值248, 本地值247] = 剩余地址.split("@").reverse();
  let 本地值246, 密码245, 主机名244, 链路端口;
  if (本地值247) {
    const 本地值243 = 本地值247.split(":");
    if (本地值243.length !== 2) throw new Error(错误_无效链路地址);
    [本地值246, 密码245] = 本地值243;
  }
  ...
  if (isNaN(链路端口)) {
    if (链路种类 === 链路种类_套接字5) throw new Error(错误_无效链路地址);
    本地值242.push(末段值);
    链路端口 = 链路种类 === 链路种类_安全隧道 ? 443 : 80;
  }
  主机名244 = 本地值242.join(":");
  if (!主机名244) throw new Error(错误_无效链路地址);
  if (主机名244.includes(":") && !/^\[.*\]$/.test(主机名244)) throw new Error(错误_无效链路地址);
  return {
    username: 本地值246,
    password: 密码245,
    hostname: 主机名244,
...
async function 处理订阅值(请求241, 用户240 = null) {
  ...
  const 本地值235 = {
    zh: {
      ...
      generateIP: '生成IP',
      fetchIP: '获取IP',
      linkConfig: 解码64('5Luj55CG6YWN572uIChzKTo='),
    ...
    fa: {
      ...
      generateIP: 'تولید IP',
      fetchIP: 'دریافت IP',
      linkConfig: 解码64('2KrZhti424zZhdin2Kog2b7YsdmI2qnYs9uMIChzKTo='),
...
async function 解析木马头部(缓冲234, 本地值233) {
  ...
  const 链路5数据缓冲 = 字节.subarray(值值索引 + 2);
  if (链路5数据缓冲.byteLength < 6) {
    return {
      hasError: true,
      message: atob('aW52YWxpZCBTT0NLUzUgcmVxdWVzdCBkYXRh')
    };
  }
  const 视图231 = new DataView(链路5数据缓冲.buffer, 链路5数据缓冲.byteOffset, 链路5数据缓冲.byteLength);
  ...
  switch (本地值229) {
    case 1:
      地址长度 = 4;
      地址227 = 链路5数据缓冲.subarray(地址索引228, 地址索引228 + 地址长度).join(".");
      break;
    case 3:
      地址长度 = 链路5数据缓冲[地址索引228];
      地址索引228 += 1;
      地址227 = 共享解码器.decode(链路5数据缓冲.subarray(地址索引228, 地址索引228 + 地址长度));
      break;
    case 4:
      地址长度 = 16;
      const 数据视图 = new DataView(链路5数据缓冲.buffer, 链路5数据缓冲.byteOffset + 地址索引228, 地址长度);
      const 值6 = [];
  ...
  const 端口远程 = new DataView(链路5数据缓冲.buffer, 链路5数据缓冲.byteOffset + 端口索引225, 2).getUint16(0);
  return {
    hasError: false,
    addressRemote: 地址227,
    addressType: 本地值229,
    port: 端口远程,
    hostname: 地址227,
    rawClientData: 链路5数据缓冲.subarray(端口索引225 + 4)
  };
}
...
// 用一个远程套接字装配上下行器：uploader 负责写 trans 首包与后续上行，downloader 回灌 resp+下行
...
// 出站决策与 ws 的 处理值值384 对齐：接入 s（链路）、wk/rm（地区匹配备用地址）、qj（链路降级/仅走链路）
async function 连接值远程扩展超文本(首包, 请求值扩展 = null) {
  const 主机 = 首包.hostname;
  const 端口 = 首包.port;
  const 直连 = async (地址, 端口值) => 连接值套接字(地址, 端口值, 请求值扩展, 传输连接竞速数);
  const 走链路 = async (地址, 端口值) => 处理值链路连接(地址类型_网址, 地址, 端口值, 已解析链路5配置, 请求值扩展, null);
  ...
  const 首跳走链路 = 仅走链路 && 是否链路已启用 ? true : 启用链路降级 ? false : 是否链路已启用;
  try {
    const 套接字 = 首跳走链路 ? await 走链路(主机, 端口) : await 直连(主机, 端口);
    return 装配扩展超文本连接(首包, 套接字);
  } catch (首跳错误) {
    // 只走链路：首跳失败不回落直连，避免出口 IP 泄漏
    if (仅走链路 && 是否链路已启用) return null;
    try {
      if (启用链路降级 && 是否链路已启用) {
        try {
          const 链路套接字 = await 走链路(主机, 端口);
          return 装配扩展超文本连接(首包, 链路套接字);
        } catch (链路错误) {
      ...
      const 回退 = await 取回退目标();
      const 回退套接字 = 是否链路已启用 ? await 走链路(回退.address, 回退.port) : await 直连(回退.address, 回退.port);
      return 装配扩展超文本连接(首包, 回退套接字);
    } catch (回退错误) {
      return null;
    }
